...
FLASK_URL = "http://localhost:5000/gesture_detected"
...
def get_trace_direction(points, min_movement=40):
    if len(points) < 2:
...
def decide_action(gesture_name, direction):
...
while True:
    ...
    if locked and no_trace_frames >= NO_TRACE_FRAME_LIMIT:
        ...
        direction = get_trace_direction(trace_points)
        print(f"Trace direction: {direction}")
        ...
        if best_template is not None and best_score < SIMILARITY_THRESHOLD:
            ...
            action = decide_action(gesture_name, direction)

            print("MATCH FOUND")
            print(f"Matched template: {gesture_name}")
            ...
            try:
                ...
                if response.status_code == 200:
                    print("Flask notified about no match")
                    print(response.json())
